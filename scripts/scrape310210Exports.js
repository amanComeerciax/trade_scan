/**
 * 🚢 TradeScan 100% BULLETPROOF ENGINE - HS 310210 WORLD EXPORTERS
 * ==================================================================================
 * Built on the exact proven architecture of HS 310210 Imports & HS 5208 World:
 * 
 *   1. Bina Ruke (Zero Crashes / Zero Bans):
 ...
 *        coverage of all Exporters past the 7,000 Elasticsearch offset limit.
 * 
 ...
 *      - Trade Flow = 'Exporter' (or 'Both' if already an Importer)
 * 
 *   3. Relational Deduplication:
 *      - Unique trademapId & @@unique([name, country]) upsert into Prisma.
 *      - Linked to CompanyProduct table with HS 310210 Urea & Fertilizers (tradeType: 'Exporter').
...
const CONFIG = {
  ...
  TRADE_FLOW: 'E',     // E = Exports (EXPORTERS)
  ...
  CHECKPOINT_FILE: path.join(process.env.LOCALAPPDATA || '', 'TradeScan-checkpoint-310210-exports-full.json'),
};
...
// ============================================================
// CHECKPOINT STORAGE (CONTINUOUS PROGRESS RESUMPTION)
// ============================================================
...
async function runScrapePass({
  ...
  let totalPages = Math.ceil(totalRecordsGoal / CONFIG.PAGE_SIZE) || 85;
  ...
  while (currentPage <= totalPages) {
    ...
    console.log(`\n📄 [TradeMap ${sortDir.toUpperCase()}] Page ${currentPage}/${totalPages} | Received ${records.length} Exporters (Total available: ${totalRecordsGoal})`);
    ...
    // 2. Process each company & fetch Contact Person + Direct Phone (Bulletproof 1-by-1)
    for (let i = 0; i < records.length; i++) {
      ...
      const counterStr = `[${stats.processed}/${totalRecordsGoal || '8500'}]`;
...
async function main() {
  console.log('\n' + '='.repeat(72));
  console.log(`🚢 TradeScan BULLETPROOF ENGINE - HS 310210 EXPORTERS (WORLD)`);
  console.log('='.repeat(72));
  console.log('🛡️ Exact Proven Architecture: Safe Sequential Pacing | Zero Burst Bans');
  ...
  if (checkpoint && checkpoint.lastPage && checkpoint.lastPage > 0) {
    console.log(`\n💾 Saved Checkpoint Detected!`);
    console.log(`   Last Completed Page: ${checkpoint.lastPage} (${checkpoint.pass.toUpperCase()})`);
    console.log(`   Processed So Far: ${checkpoint.totalProcessed} (New: ${checkpoint.totalInserted})`);
    const resumeAns = await ask(`👉 Checkpoint se resume karein (Page ${checkpoint.lastPage + 1})? [Y/n]: `);
    if (resumeAns.toLowerCase() !== 'n') {
    ...
    console.log(`\n📌 Target: ALL HS 310210 EXPORTERS (Fresh from Page 1 with 100% full contact details)`);
    ...
    if (chosenNum && chosenNum >= 1 && chosenNum <= 150) {
      startPage = chosenNum;
  ...
  if (currentPass === 'asc') {
    ...
    if (pass1Result.reachedCeiling) {
      ...
      console.log('   Elasticsearch offset limit bypassed. Capturing remaining Exporters...');
      console.log('═'.repeat(72) + '\n');
  ...
  // Grand Summary
  console.log('\n' + '='.repeat(72));
  console.log(`🎉 ALL HS ${CONFIG.HS_CODE} EXPORTERS EXTRACTION COMPLETED!`);
