...
const CONFIG = {
  ...
  PROFILE_DIR: fs.existsSync(path.join(__dirname, '..', '.session-cache', 'trademap-profile'))
    ? path.join(__dirname, '..', '.session-cache', 'trademap-profile')
...
async function main() {
  ...
  if (cli.fresh || (checkpoint && checkpoint.nbPages > 1000)) {
    clearCheckpoint(hsCode, countryCode, tradeFlow, sortDir);
    console.log(`🆕 Starting fresh from Page 1.\n`);
  } else if (checkpoint && checkpoint.lastSuccessfulPage > 0) {
  ...
  const launchOptions = {
    headless: cli.headless !== 'false' && cli.headless !== false,
    args: [
      '--no-sandbox',
  ...
  const context = await chromium.launchPersistentContext(CONFIG.PROFILE_DIR, launchOptions);
  ...
  }).catch(() => ({ isBlocked: false, isSignIn: false, url: '' }));
  ...
  if (sessionCheck.isBlocked || sessionCheck.isSignIn) {
    ...
    // Reload page to apply session cookies and capture authenticated headers
    console.log(`🔄 Reloading TradeMap origin to capture authenticated session...`);
    await page.goto(initialUrl, { waitUntil: 'networkidle', timeout: 45000 }).catch(async () => {
  ...
  const tokenDetected = await page.evaluate(() => {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        const val = JSON.parse(localStorage.getItem(k));
        if (val?.authnResult?.access_token || val?.access_token) return true;
      } catch {}
      const raw = localStorage.getItem(k);
      if (typeof raw === 'string' && raw.startsWith('eyJ')) return true;
    }
    return false;
  }).catch(() => false);

  if (tokenDetected) {
    console.log(`🔑 Authenticated Session Confirmed! (Bearer Token Active)\n`);
  } else {
    console.log(`ℹ️  Proceeding with session cookies.\n`);
  }

  console.log(`🚀 TradeMap JSON API Engine active. Starting pagination loop...\n`);
  await page.waitForLoadState('networkidle').catch(() => {});
  await sleep(2000);
  ...
  while (true) {
    ...
    // Process & Upsert every object inside records[] with fast concurrency + live counter
    let pageInserted = 0;
    let pageUpdated = 0;
    ...
    // Concurrent batching (5 records at a time for fast network saves)
    let savedSoFar = 0;
    const batchSize = 5;
    for (let i = 0; i < normalizedRecords.length; i += batchSize) {
      const chunk = normalizedRecords.slice(i, i + batchSize);
      const chunkResults = await Promise.all(chunk.map((norm) => upsertCompanyRecord(norm)));
      ...
      process.stdout.write(`\r   💾 Saving to database: [${savedSoFar}/${normalizedRecords.length}] companies...`);
    }

    process.stdout.write(`\r   💾 Saved to database: [${normalizedRecords.length}/${normalizedRecords.length}] companies. Done!   \n`);
